...
--  Changes: UUID → VARCHAR(64), SESSION_LEG table added,
--           TRANSFER_EVENT table added, messages link to leg
-- ============================================================

-- ── TABLE 1: CHAT_SESSION  (ROOT)
CREATE TABLE chat_session (
    session_id   VARCHAR(64)       NOT NULL VARCHAR(64) APP-GENERATED,
...
CREATE TABLE mobile_device (
    device_id          VARCHAR(64)       NOT NULL VARCHAR(64) APP-GENERATED,
...
CREATE TABLE ucid_mapping (
    ucid_map_id       VARCHAR(64)       NOT NULL VARCHAR(64) APP-GENERATED,
    session_id        VARCHAR(64)       NOT NULL,
...
CREATE TABLE session_leg (
    leg_id            VARCHAR(64)       NOT NULL VARCHAR(64) APP-GENERATED,
    session_id        VARCHAR(64)       NOT NULL,
    ...
    bot_id            VARCHAR(64),
    -- transfer chain
    prev_leg_id       VARCHAR(64),                         -- NULL for first leg; FK to self
...
CREATE TABLE transfer_event (
    transfer_id       VARCHAR(64)       NOT NULL VARCHAR(64) APP-GENERATED,
    session_id        VARCHAR(64)       NOT NULL,
    from_leg_id       VARCHAR(64)       NOT NULL,
    to_leg_id         VARCHAR(64),                         -- NULL until receiving leg is created
...
CREATE TABLE chat_message (
    message_id   VARCHAR(64)      NOT NULL VARCHAR(64) APP-GENERATED,
    session_id   VARCHAR(64)      NOT NULL,
    ...
    sender_id    VARCHAR(64),                              -- customer_id / agent_id / supervisor_id / bot_id
...
CREATE TABLE disconnect_event (
    disconnect_id    VARCHAR(64)    NOT NULL VARCHAR(64) APP-GENERATED,
    session_id       VARCHAR(64)    NOT NULL,
...
CREATE TABLE license_event (
    license_event_id  VARCHAR(64)    NOT NULL VARCHAR(64) APP-GENERATED,
    session_id        VARCHAR(64),                         -- NULLABLE: pre-session events
    mobile_id         VARCHAR(64),
...
CREATE TABLE leg_disconnect (
    leg_disconnect_id  VARCHAR(64)       NOT NULL VARCHAR(64) APP-GENERATED,
    leg_id             VARCHAR(64)       NOT NULL,          -- the leg that ended
    session_id         VARCHAR(64)       NOT NULL,          -- denorm for direct session queries
    participant_type   VARCHAR(16)  NOT NULL
                                    CHECK (participant_type IN ('customer','agent','supervisor','bot')),
    ...
    transfer_id        VARCHAR(64),                         -- FK to transfer_event (nullable)
    next_leg_id        VARCHAR(64),                         -- the leg that replaced this one
    raw_payload        JSONB,
